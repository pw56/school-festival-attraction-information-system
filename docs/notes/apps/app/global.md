...
- テキスト、代替テキスト、a11y用ラベル、開設用動画のパスなど、全てreact-i18nextのi18n関数に置き換えながら実装(文章は1文字も変えずにi18nの翻訳ファル化)
- Figmaにもあるように、アプリ内は全て`Kosugi Maru`フォント
...
- SVGにも該当しない図形はdivで実装(状況に応じて判断し、a11y用の属性も付与)
- Reactコンポーネント作成時はスプレッド構文で、コンポーネントが使わないpropsをそのまま流して割り当てる(`...rest`で実装)
